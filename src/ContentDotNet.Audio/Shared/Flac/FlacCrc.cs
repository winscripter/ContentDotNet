namespace ContentDotNet.Audio.Shared.Flac
{
    ...
    ///   FLAC CRC hasher.
    /// </summary>
    public static class FlacCrc
    {
        /// <summary>
        ///   The CRC8 table.
        /// </summary>
        public static ReadOnlySpan<byte> Crc8Table => [
        ...
        /// <summary>
        ///   Computes the CRC-8 hash.
        /// </summary>
        /// <param name="sourceData">The data to hash.</param>
        /// <param name="startOffset">The initial offset of data.</param>
        /// <returns>The CRC-8 hash.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static byte ComputeCrc8Hash(ReadOnlySpan<byte> sourceData, int startOffset)
        {
            byte crc = 0;
            int i = startOffset;
            while (i < sourceData.Length)
                crc = Crc8Table[crc ^ sourceData[i++]];
            return crc;
        }

        public static ushort ComputeCrc16Hash(ReadOnlySpan<byte> sourceData, int startOffset)
        {
            ushort crc = 0;
            int i = startOffset;
            while (i < sourceData.Length)
            {
                crc ^= (ushort)(sourceData[i] << 8 | sourceData[i + 1]);

                crc = (ushort)(Crc16Table[7, crc >> 8] ^ Crc16Table[6, crc & 0xFF] ^
                      Crc16Table[5, sourceData[i + 2]] ^ Crc16Table[4, sourceData[i + 3]] ^
                      Crc16Table[3, sourceData[i + 4]] ^ Crc16Table[2, sourceData[i + 5]] ^
                      Crc16Table[1, sourceData[i + 6]] ^ Crc16Table[0, sourceData[i + 7]]);
