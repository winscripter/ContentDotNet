...
/// <summary>
/// Represents a 4x16x2 container matrix backed by a <see cref="Container32UInt32"/>.
/// </summary>
public struct ContainerMatrix4x16x2 : IEquatable<ContainerMatrix4x16x2>
{
    /// <summary>
    /// The underlying data storage for the matrix.
    /// </summary>
    private Container128UInt32 m_Data;

    /// <summary>
    /// Initializes a new instance of the <see cref="ContainerMatrix4x16x2"/> struct with the specified data.
    /// </summary>
    /// <param name="data">The data to initialize the matrix with.</param>
    public ContainerMatrix4x16x2(Container128UInt32 data) => m_Data = data;

    /// <summary>
    /// Initializes a new instance of the <see cref="ContainerMatrix4x16x2"/> struct with default data.
    /// </summary>
    public ContainerMatrix4x16x2()
        : this(new())
    {
    }
    ...
    {
        get => m_Data[(int)(a * 32 + b * 2 + c)];
        set => m_Data[(int)(a * 32 + b * 2 + c)] = value;
    }
    ...
    {
        get => (int)m_Data[a * 32 + b * 2 + c];
        set => m_Data[a * 32 + b * 2 + c] = (uint)value;
    }
    ...
    {
        return obj is ContainerMatrix4x16x2 x && Equals(x);
    }
    ...
    public readonly bool Equals(ContainerMatrix4x16x2 other)
    {
    ...
    /// <summary>
    /// Determines whether two <see cref="ContainerMatrix4x16x2"/> instances are equal.
    /// </summary>
    /// <param name="left">The first matrix to compare.</param>
    /// <param name="right">The second matrix to compare.</param>
    /// <returns><c>true</c> if the matrices are equal; otherwise, <c>false</c>.</returns>
    public static bool operator ==(ContainerMatrix4x16x2 left, ContainerMatrix4x16x2 right)
    {
        return left.Equals(right);
    }

    /// <summary>
    /// Determines whether two <see cref="ContainerMatrix4x16x2"/> instances are not equal.
    /// </summary>
    /// <param name="left">The first matrix to compare.</param>
    /// <param name="right">The second matrix to compare.</param>
