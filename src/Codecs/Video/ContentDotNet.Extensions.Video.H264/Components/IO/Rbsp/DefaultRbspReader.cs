    ...
    internal class DefaultRbspReader : IH264IOReader
    {
        ...
        public async Task<RbspResidual> ReadResidualAsync(IH264SyntaxReader syntaxReader, H264MacroblockInfo mb, H264RbspState rbspState)
        {
            ...
            if (!Grabber.GetEntropyCodingModeFlag(rbspState))
                ...
                residual_block = ReadCabacResidualAsync;
            ...
            await ResidualLuma(Intra16x16DCLevel, Intra16x16ACLevel, LumaLevel4x4, LumaLevel8x8, startIdx, endIdx);
            ...
            if (rbspState.ChromaArrayType() is 1 or 2)
            {
                ...
                for (int iCbCr = 0; iCbCr < 2; iCbCr++)
                {
                    if ((mb.Rbsp.GetCodedBlockPatternChroma() & 3).AsBoolean() && startIdx == 0)
                    {
                        await residual_block(ChromaDCLevel[iCbCr], 0, 4 * NumC8x8 - 1, 4 * NumC8x8);
                    }
                }
                ...
                for (int iCbCr = 0; iCbCr < 2; iCbCr++)
                {
                    for (int i8x8 = 0; i8x8 < NumC8x8; i8x8++)
                    {
                        ...
                        {
                            if ((mb.Rbsp.GetCodedBlockPatternChroma() & 2).AsBoolean())
                                await residual_block(ChromaACLevel[iCbCr][i8x8 * 4 + i4x4], Math.Max(0, startIdx - 1), endIdx - 1, 15);
                            else
                                for (int i = 0; i < 15; i++)
                                    ChromaACLevel[iCbCr][i8x8 * 4 + i4x4][i] = 0;
                        }
            ...
            {
                await ResidualLuma(CbIntra16x16DCLevel, CbIntra16x16ACLevel, CbLevel4x4, CbLevel8x8, startIdx, endIdx);
                await ResidualLuma(CrIntra16x16DCLevel, CrIntra16x16ACLevel, CrLevel4x4, CrLevel8x8, startIdx, endIdx);
            }
            ...
            async Task ResidualLuma(DcLevel16x16 i16x16DClevel, AcLevel16x16 i16x16AClevel, Level4x4 level4x4, Level8x8 level8x8,
                    int startIdx, int endIdx)
            {
                if (startIdx == 0 && MacroblockTraits.MbPartPredMode(mb, 0) == Intra_16x16)
                    await residual_block(i16x16DClevel.Value, 0, 15, 16);
                ...
                for (int i8x8 = 0; i8x8 < 4; i8x8++)
                {
                    if (!mb.Rbsp.TransformSize8x8Flag || !Grabber.GetEntropyCodingModeFlag(rbspState))
                    {
                        for (int i4x4 = 0; i4x4 < 4; i4x4++)
                        {
                            if ((mb.Rbsp.GetCodedBlockPatternLuma() & (1 << i8x8)).AsBoolean())
                            {
                                if (MacroblockTraits.MbPartPredMode(mb, 0) == Intra_16x16)
                                {
                                    await residual_block(i16x16AClevel.Value[i8x8 * 4 + i4x4], Math.Max(0, startIdx - 1), endIdx - 1, 15);
                                }
                                else
                                {
                                    await residual_block(level4x4.Value[i8x8 * 4 + i4x4], startIdx, endIdx, 16);
                                }
                            }
                            else if (MacroblockTraits.MbPartPredMode(mb, 0) == Intra_16x16)
                            {
                    ...
                    {
                        await residual_block(level8x8.Value[i8x8], 4 * startIdx, 4 * endIdx + 3, 64);
                    }
                    else
                    {
                        for (int i = 0; i < 64; i++)
            ...
            async Task ReadCavlcResidualAsync(List<int> coeffLevel, int start, int end, int maxNumCoeff)
            {
                // TODO: CAVLC Residuals
            ...
            async Task ReadCabacResidualAsync(List<int> coeffLevel, int start, int end, int maxNumCoeff)
            {
                bool cbf = (maxNumCoeff != 64 || rbspState.ChromaArrayType() == 3) && syntaxReader.ReadCodedBlockFlag();
                coded_block_flag.Value = cbf;
                ...
                if (cbf)
                {
                    bool[] significant_coeff_flag = new bool[end + 1];
                    bool[] last_significant_coeff_flag = new bool[end + 1];
                    ...
                    while (i < numCoeff - 1)
                    {
                        significant_coeff_flag[i] = await syntaxReader.ReadSignificantCoeffFlagAsync();
                        if (significant_coeff_flag[i])
                        {
                            last_significant_coeff_flag[i] = await syntaxReader.ReadLastSignificantCoeffFlagAsync();
                            if (last_significant_coeff_flag[i])
                                numCoeff = i + 1;
                        }
