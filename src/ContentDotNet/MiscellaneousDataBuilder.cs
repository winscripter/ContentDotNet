   ...
   IEnumerable<IMiscellaneousData>
{
    ...
    {
        if (ContainsData(miscellaneousData))
            throw new InvalidOperationException("Data already exists");
        _miscellaneousDataBacking.Add(miscellaneousData);
    }
    ...
    {
        if (!ContainsData(with))
            throw new InvalidOperationException("No element with category " + with.DisplayName);

        IMiscellaneousData matching = _miscellaneousDataBacking.First(e => e.DisplayName == with.DisplayName);
        _miscellaneousDataBacking.Remove(matching);

        _miscellaneousDataBacking.Add(with);
    }

    /// <summary>  
    ///   Gets the count of miscellaneous data items in the builder.  
    /// </summary>  
    public int Count => _miscellaneousDataBacking.Count;
    ...
    {
        return _miscellaneousDataBacking.Any(e => e.DisplayName == category);
    }
    ...
    public IMiscellaneousData GetData(string category)
    {
        ...
        return _miscellaneousDataBacking.First(e => e.DisplayName == category);
    }
    ...
    {
        return _miscellaneousDataBacking.FirstOrDefault(e => e.DisplayName == category);
    }
